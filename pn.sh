...
##########################
# PURENEXUS BUILD SCRIPT #
##########################
# Build PureNexus with one easy script. You will need to have the repo synced and configured already, use this page to help you with that: https://raw.githubusercontent.com/nathanchance/Android-Tools/master/Building_AOSP.txt
...
# Usage #
#########
# $ . pn.sh <device> <sync|nosync> <clean|noclean> <log|nolog>
...
# Examples #
############
# $ . pn.sh angler sync clean nolog
# $ . pn.sh hammerhead nosync noclean log
...
# Variables #
#############
# SOURCEDIR: The directory that holds your PN repos (for example, /home/<username>/android/PN, this must be changed)
...
# DESTDIR: The directory that will hold your completed PN zip files for ease of access (for example, /home/<username>/completed_zips, this must be changed)
...
# EDIT OPTION
# PURENEXUS_BUILD_TYPE section: Add text after the equals sign if you want something other than HOMEMADE in the PureNexus version (under the About Phone section)
...
if [[ "${LOG}" == "log" ]]; then
   ...
   time mka bacon 2>&1 | tee ${LOGDIR}/pn_${DEVICE}-${NOW}.log
else
   time mka bacon
fi
